public class ACPWrapper {
  ...
  /*========| Public Methods |========*/
  public string[] Main(string Argument, out long senderId) {                  //Main method, call each tick/when you received a msg.
    //Each tick
    SendMSG();
  ...
  public bool PrepareMSG(string[] parameters, string To, bool anon = false) { //Prepares a message, returns false if it fails.
    StringBuilder constructedMSG = new StringBuilder();
  ...
  public void SetLaserTarget(string GPS) { //Expects gps in format GPS:Name:X:Y:Z:
  ...
  private bool IsDirectedAtMe(string Head, out long SenderID) {
    ...
    if (headparts.Length == 2) {
      ...
      if (Int64.TryParse(headparts[1], out MyID)) {  //Parses My ID
